...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildDot(cx, cy, generation) {
    const el = document.createElementNS(ns, 'circle');
    el.setAttribute('cx', cx.toFixed(2));
    el.setAttribute('cy', cy.toFixed(2));
    el.setAttribute('r', '6.5');
    el.setAttribute('class', 'tree-node tree-joint-dot');
    el.dataset.generation = String(generation);
    svg.appendChild(el);
  }
  ...
  function applyStage() {
    ...
    svg.querySelectorAll('.tree-link-segment, .tree-joint-dot').forEach((el) => {
  ...
  function draw() {
    ...
    stageGroups.forEach((group) => {
      ...
      const parentStemTop = Math.max(leftParent.box.bottom, rightParent.box.bottom) - 2;
      const parentStemY = parentStemTop + Math.max(18, Math.min(30, children[0].box.top - parentStemTop - 46));
      const unionCenterX = (leftParent.box.centerX + rightParent.box.centerX) / 2;
      const splitY = Math.min(
        Math.min(...children.map(({ box }) => box.top)) - 28,
        parentStemY + Math.max(54, children[0].box.height * 0.8)
      );
      const clampedSplitY = Math.max(parentStemY + 34, splitY);

      buildLine(leftParent.box.centerX, parentStemTop, leftParent.box.centerX, parentStemY, 'tree-drop', childGeneration);
      buildLine(rightParent.box.centerX, parentStemTop, rightParent.box.centerX, parentStemY, 'tree-drop', childGeneration);
      buildLine(leftParent.box.centerX, parentStemY, rightParent.box.centerX, parentStemY, 'tree-union', childGeneration);
      buildDot(unionCenterX, parentStemY, childGeneration);
      buildLine(unionCenterX, parentStemY, unionCenterX, clampedSplitY, 'tree-drop', childGeneration);

      if (children.length === 1) {
        const child = children[0].box;
        const childAnchorY = child.top - 14;
        buildLine(unionCenterX, clampedSplitY, child.centerX, clampedSplitY, 'tree-connector', childGeneration);
        buildLine(child.centerX, clampedSplitY, child.centerX, childAnchorY, 'tree-drop', childGeneration);
        buildDot(child.centerX, clampedSplitY, childGeneration);
      } else {
        const leftChildX = children[0].box.centerX;
        const rightChildX = children[children.length - 1].box.centerX;
        buildLine(leftChildX, clampedSplitY, rightChildX, clampedSplitY, 'tree-connector', childGeneration);
        buildDot(unionCenterX, clampedSplitY, childGeneration);
        children.forEach(({ box }) => {
          const childAnchorY = box.top - 14;
          buildLine(box.centerX, clampedSplitY, box.centerX, childAnchorY, 'tree-drop', childGeneration);
        });
      }
    });
